...
──────────────────────────
Builds the TE Advanced Stats datasets for the StatChasers frontend.

Produces five output files (mirrors the QB/RB/WR advanced builders, and uses the
same column set as the WR Advanced Stats dataset):
  output/te_advanced_stats_2023.json   — 2023 season only
  output/te_advanced_stats_2024.json   — 2024 season only
  output/te_advanced_stats_2025.json   — 2025 season only
  output/te_advanced_stats_all.json    — 2023 + 2024 + 2025 combined (one row per player)
  output/te_advanced_stats.json        — alias for 2025 (backward compat)

A target-volume / efficiency / separation leaderboard covering every TE with at
...
Definitions match the canonical TE efficiency tab so a TE reports consistent
...
# ---------------------------------------------------------------------------
# Column definitions (identical to the WR Advanced Stats schema)
# ---------------------------------------------------------------------------
...
def build_season(
    ...
    te = stats[(stats["position"] == POSITION) & (stats["season"] == season)].copy()
    ...
    for _, s in te.iterrows():
        ...
        rows.append({
            "playerId":                     resolver.resolve(name, team, POSITION),
            "playerName":                   name,
            "position":                     POSITION,
            "team":                         team,
...
def _validate(rows: list[dict], label: str) -> None:
    ...
    if no_id:
        print(f"  NOTE: {len(no_id)} TE(s) without a Sleeper playerId in {label}: "
              f"{', '.join(no_id[:8])}{'…' if len(no_id) > 8 else ''}", file=sys.stderr)
    print(f"  Validation OK for {label}: {len(rows)} TEs.")
...
def main() -> None:
    ...
    multi_path = OUTPUT_DIR / "te_advanced_stats_all.json"
    ...
    alias_path = OUTPUT_DIR / "te_advanced_stats.json"
    shutil.copy2(OUTPUT_DIR / "te_advanced_stats_2025.json", alias_path)
    print(f"Wrote {alias_path} (alias for 2025)")

    print("\nTE advanced stats build complete.")
